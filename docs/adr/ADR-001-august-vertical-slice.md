...
Hathor must deliver a local demonstrable microservice product by August 15, 2026. The architecture must prioritize reliable payment-to-entitlement behavior, secure creator customization, constrained AI features, and reproducible local operations without claiming production scale or live-payment settlement.
...
3. The August release is developed and demonstrated locally with Docker Compose. VPS/public staging is deferred until hosting is budgeted; when it resumes, it uses the same Compose topology, a TLS reverse proxy, private Docker networks, persistent service volumes, and only ports 80/443 publicly exposed.
...
8. The Store Page Designer AI is a catalog-service provider adapter. It uses an optional OpenAI key locally and returns only schema-constrained JSON Patch proposals. A catalog-owned local recommendation capability uses cached catalog metadata and deterministic fallback; it does not add a new service, profiling, or browse-time LLM calls. Manual designer functionality remains available without an AI provider.
