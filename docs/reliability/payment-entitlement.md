...
## Future Architecture & Reliability Improvements (Issue M2.4)

1. **Pre-Fetched Single-Phase Atomic Transaction:**
   - **RATIONALE:** Current implementation uses a two-phase DB transaction where `pg_advisory_xact_lock` releases at Phase 1 commit, leaving an unlocked window during external HTTP calls (`getCatalogQuotes`, `checkLibraryOwnership`) before Phase 2.
   - **PROPOSED DESIRED STATE:** Perform read-only inter-service HTTP calls before opening any DB transaction. Once quotes and ownership facts are fetched, execute a single DB transaction that locks the cart `FOR UPDATE`, checks `cartVersion`, inserts the order and idempotency record, and clears the cart atomically within < 5 ms.
   - **REVERSIBILITY & BACKWARD COMPATIBILITY:** Fully backwards-compatible; revertible to two-phase model via route config if inter-service latency patterns dictate.

2. **Composite Primary Key `(user_id, key)` for Idempotency Records:**
   - **RATIONALE:** The `idempotency_records` table currently uses a global single primary key `key`, allowing cross-user key collisions and potential Denial of Service (DoS) if User B submits a key previously generated by User A.
   - **PROPOSED DESIRED STATE:** Update Drizzle schema to primary key `(user_id, key)` to guarantee strict user-scoped idempotency isolation.
   - **REVERSIBILITY:** Simple schema migration dropping and re-adding index constraints.

3. **Deterministic Constraint Exception Handling:**
   - **RATIONALE:** Broad try/catch fallback handlers mask unexpected system failures.
   - **PROPOSED DESIRED STATE:** Inspect PostgreSQL error code `23505` (Unique Constraint Violation) explicitly to distinguish idempotency replays from runtime exceptions.

4. **Frontend Payment Tab & Engine State Synchronization:**
   - **RATIONALE:** Credit Card and PayPal UI tabs in checkout are visual shells defaulting to `sim_fawry`.
   - **PROPOSED DESIRED STATE:** Bind active tab state directly to the payment mutation payload or explicitly mark non-simulator tabs as sandbox disabled.
